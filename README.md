...
- **A scaled resolution costs you the hardware cursor, so OpenDisplay switches the pointer to software while one is up.** Measured while mirrored: `CGDisplayIsActive` is **1** for the virtual display and **0** for the real panel, which is a hardware mirror slave. macOS composites the pointer onto the active display's pipe, and that pipe scans out to nothing; the panel receives a framebuffer copy, and the hardware cursor was never in the framebuffer. That is why the pointer disappears in games at a scaled resolution. **The fix is the cursor scale.** The hardware cursor plane only carries 1x and 2x pointer bitmaps, so any other scale makes the window server draw the pointer into the framebuffer instead, where the mirror can show it. `SLSHardwareCursorActive` reports which path is live, and it flips at the boundary: scale 1.0 and 2.0 are hardware, 2.0001 and 2.5 and 3.0 are software. OpenDisplay nudges the scale by 0.0001 when a scaled mode goes up and puts it back on teardown, which is 0.005% larger and under a hundredth of a pixel on the glyph. **Two costs, both measured.** A software pointer is composited per frame rather than by the display pipe, so it can lag under heavy GPU load. And once the window server has switched to the software path it does not switch back for the rest of the login session: restoring the scale, and even a full mode set, leave `SLSHardwareCursorActive` false. Log out to get the hardware pointer back. **Reversing the mirror is not an alternative:** with the panel as master its pipe goes active and the pointer returns, but the mode drops to 2560x1440 at 1:1, because the master drives the mode and the panel has no oversized mode of its own.
